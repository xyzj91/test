...
function time_format($time = NULL,$format='Y-m-d H:i'){
    $time = $time === NULL ? getTime() : intval($time);
...
function get_nickname($uid = 0){
    ...
    if(isset($list[$key])){ //已缓存，直接使用
        ...
        $info = model('Member')->field('nickname')->find($uid);
...
function get_top_model($model_id=null){
    ...
    $model = model('Model')->where($map)->field(true)->select();
...
function action_log($action = null, $model = null, $record_id = null, $user_id = null){
    ...
    if(!empty($action_info['log'])){
        if(preg_match_all('/\[(\S+?)\]/', $action_info['log'], $match)){
            ...
            $log['model']   =   $model;
            $log['time']    =   getTime();
            $log['data']    =   array('user'=>$user_id,'model'=>$model,'record'=>$record_id,'time'=>getTime());
...
function execute_action($rules = false, $action_id = null, $user_id = null){
    ...
    foreach ($rules as $rule){
        ...
        $map['create_time'] = array('gt', getTime() - intval($rule['cycle']) * 3600);
...
function get_model_attribute($model_id, $group = true,$fields=true){
    ...
    if(!isset($list[$model_id])){
        ...
        $extend = model('Model')->getFieldById($model_id,'extend');

        if($extend){
            $map = array('model_id'=> array("in", array($model_id, $extend)));
        }
        $info = model('Attribute')->where($map)->field($fields)->select();
        $list[$model_id] = $info;
    }
...
function get_table_field($value = null, $condition = 'id', $field = null, $table = null){
    ...
    //拼接参数
    $map[$condition] = $value;
    $info = model(ucfirst($table))->where($map);
    if(empty($field)){
