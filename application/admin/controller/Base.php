...
class Base extends Controller{
	...
	*/
    ...
    final protected function checkRule($rule, $type, $mode='url'){
        static $Auth    =   null;
		$type = isset($type)?$type:AuthRule::RULE_URL;
        if (!$Auth) {
            $Auth       =   new Auth();
        }
    ...
    public function setStatus($Model){
		...
		$Model = isset($Model)?$Model:CONTROLLER_NAME;
        if(empty($ids)){
    ...
    final public function getMenus($controller=null){
    	$controller = isset($controller)?$controller:CONTROLLER_NAME;
        ...
        if(empty($menus)){
            ...
            // 查找当前子菜单
            $pid = DB::name('Menu')->where("pid !=0 AND url like '%{$controller}/".ACTION_NAME."%'")->field('pid')->select();
            if($pid){
                // 查找当前主菜单
                ...
                foreach ($menus['main'] as $key => $item) {
                    ...
                    if($item['id'] == $nav['id']){
                        $menus['main'][$key]['class']='current';
                        //生成child树
                        $groups = DB::name('Menu')->where(array('group'=>array('neq',''),'pid' =>$item['id']))->distinct(true)->getField("group",true);
                        //获取二级分类的合法url
                        $where          =   array();
                        $where['pid']   =   $item['id'];
                        $where['hide']  =   0;
                        ...
                        $second_urls = DB::name('Menu')->where($where)->getField('id,url');
	...
	*/
	public function add(){
//		$name = PREFIX.CONTROLLER_NAME;
//		$fields = DB::getTableInfo($name,'fields');
...
//      }
        ...
        import("@.Form.Form");
        $form = new Form(CONTROLLER_NAME);
        $this->assign("FormHTML", $form->makeForm());
        print_r($form->makeForm());
        $this->assign("fields", $fields);
		
		
		return $this->fetch("base/add");
	}
